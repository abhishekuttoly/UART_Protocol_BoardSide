...
const osThreadAttr_t CreatePacket_attributes = { .name = "CreatePacket",
...
int main(void) {
	...
	CreatePacketHandle = osThreadNew(PollingTask, NULL,
			&CreatePacket_attributes);
...
void PollingTask(void *argument) {
	/* USER CODE BEGIN 5 */
	_sPacketData sData;
	static uint8_t ucRxBuffer[MAX_LENGTH];
	uint8_t ucTxBuffer[MAX_LENGTH];
	uint8_t ucIndex = 0;
	uint8_t ucLength = 0;
	/* Infinite loop */
	for (;;) {
		memset(ucRxBuffer, 0, sizeof(ucRxBuffer));

		HAL_StatusTypeDef eReceiveStatus = HAL_UART_Receive(&huart3, ucRxBuffer,
				sizeof(ucRxBuffer), portMAX_DELAY);

		if (eReceiveStatus == HAL_OK) {
			printf("\nReceived data:");
			for (ucIndex = 0; ucIndex < ucRxBuffer[1] + 2; ucIndex++) {
				printf("%02X", ucRxBuffer[ucIndex]);
			}
			printf("\n");
		} else {
			printf("ERROR: %d RX timeout (normal)\n", __LINE__);
		}


		if (UARTPacketParserParsePacket(ucRxBuffer, &sData)) {
			UARTTransmitResponcePacket(&sData, &ucTxBuffer, &ucLength);
			printf("\nTransmiting data:");
			for(ucIndex =0;ucIndex<ucTxBuffer[1]+2;ucIndex++)
			{
				printf("%02x", ucTxBuffer[ucIndex]);
			}
			printf("\n");

			HAL_UART_Transmit(&huart3,
								ucTxBuffer, ucTxBuffer[1]+2, portMAX_DELAY);
		} else {
